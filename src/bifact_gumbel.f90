...
!   ! !call gumproxybifact(npar,th,mgrp,n,dvar,grsize,udata,vlat,nllk,grad,hess)
  
!   ! print *, "the negative log-likelihood is"
!   ! print *,nllk
!   ! deallocate(grsize,udata,vlat,th,grad,hess)
! END PROGRAM
...
subroutine gumproxybifact(npar,th,mgrp,n,dvar,grsize,udata,vlat,nllk,grad,hess)
  implicit none
      ...
      do jg =1,mgrp   !jth group                         
        ...
        !do iq2 =1,nq   
          ...
          do mj = ind1,ind2 ! within group index
            call cgumderivs(uvec(mj),vlat(i,1),th(mj),ccdf(mj),cder1(mj),cder2(mj)) !C_{ij|V_0}  
            call lgum2derivs(ccdf(mj),vlat(i,jg+1),th(dvar+mj),lpdf(dvar+mj),  &
             der1(dvar+mj),der2(dvar+mj),der1u(mj),der2u(mj),derumix(mj))  !c_{i,V_j;V_0} 
            call lgum1derivs(uvec(mj),vlat(i,1),th(mj),lpdf(mj),der1(mj),der2(mj)) !c_{ij,V_0} 
            lk = lk*exp(lpdf(mj)) 
