...
subroutine intpr(message, msg_len, int_val, int_val_len)
    implicit none
    ...
    ! Print the message 
    ! R check complains about this
!Compiled code should not call entry points which might terminate R nor
!write to stdout/stderr instead of to the console, nor use Fortran I/O
!nor system RNGs.
    !print *, message

    ! Optionally print the integer values if they are not zero
    if (int_val /= 0 .or. int_val_len /= 0) then
        !print *, 'Integer values:', int_val, int_val_len
    end if
