! Subroutines lt1derivs lt2derivs ctderivs are in file t12fact.f90
! Subroutines ltpdf ctcdf are in file cop12factlik.f90
...
subroutine strt2(npar,th,mgrp,n,dvar,nu,grsize,tdata,nq,wl,tl,nllk,grad,hess) 
  implicit none
    ...
    do iq =1,nq
      ...
      do jg =1,mgrp   !jth group   
        ...
        do iq2 =1,nq  
          lk = 1.d0; lk2 = 1.d0  
          do mj = ind1,ind2
            call ctderivs(tvec(mj),tl(iq,1),th(mj),nu(1),ccdf(mj),cder1(mj),cder2(mj))  !C_{ij|V_0}   						        
...
! output 
!   nllk = negative log-likelihood
subroutine strt2nllk(npar,th,mgrp,n,dvar,nu,grsize,tdata,nq,wl,tl,nllk) 
  implicit none
      ...
      do jg =1,mgrp   !jth group                         
        ...
        do iq2 =1,nq  
          ...
          do mj = ind1,ind2  ! within group index
            call ctcdf(tvec(mj),tl(iq,1),th(mj),nu(1),ccdf(mj))   !C_{ij|V_0}
            if(ccdf(mj) < 0.00001) ccdf(mj) = 0.00001
            if(ccdf(mj) > 0.99999) ccdf(mj) = 0.99999
            qtccdf = qt(ccdf(mj),nu(2))
