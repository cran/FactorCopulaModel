...
ldermixuu,lder1v,lder2v,lder2uv,ldermixvv)
  ...
  select case(icop)
    ...
    call lt2derivs(t2,t1,rho,nu,lpdf,lder1,lder2,ltder1u,ltder2u,ltdermix)
    ldermixvv(1)=ltdermix;
    ldermixvv(2)=0.0d0;
   
    lder1v=ltder1u;lder2v=ltder2u;
    ...
    ldermixvv(2)=0.0d0;
    case (7) !bb1 copula
    call lbb1derivs2(u1,u2,cparv,lpdf,lder11,lder22,lderu,lderv,lderuv)
    lder11=lder11;
    lder22=lder22;
    lder1u=lderu(1)
    lder2u=lderu(2)
    ...
    ut=1.0d0-u1
    vt=1.0d0-u2
    call lbb1derivs2(ut,vt,cparv,lpdf,lder11,lder22,lderu,lderv,lderuv)
    lder11=lder11;
    lder22=lder22;
    lder1u=-lderu(1)
    lder2u=lderu(2)
...
mderu,mderv,mder2uv)
  ...
  ! additions for second order derivatives
...
ldermixu,lder1v,lder2v,lder2uv,ldermixv)
  ...
  lder1v = -mv+mv/den+(1.d0-2.d0*cpar)*mv/m-(cpar-1.d0)/(u2*y)-1.d0/u2;
  lder2v = -m2v+m2v/den-mv*mv/den2+(1.d0-2.d0*cpar)*(m2v/m-mv*mv/msq)+(cpar-1.d0)/(u2sq*y); 
  lder2v = lder2v-(cpar-1.d0)/(y*y*u2sq)+1.d0/u2sq;
  ldermixv = -muder1+muder1/den-mv*(mder1+1.d0)/den2-2.d0*mv/m&
  +(1.d0-2.d0*cpar)*(muder1/m-mv*mder1/msq)-1.d0/(y*u2);
