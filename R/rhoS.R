...
#' @param cop function name of joint cdf or conditional cdf C_{2|1}
#' @param zero 0 or something like 1.e-6 (to avoid endpoint problems)
#' @param icond flag for using condition cdf of copula;
...
#' @param tol accuracy for 2-dimensional integration
#'
#' @return Spearman rho value for copula
#'
#' @examples
...
#'   for(iq in 1:nq)
#'   { a1 = BiCopHfunc2(xl,rep(xl[iq],nq),family=fam1,par=cpar1[1],par2=cpar1[2])
#'     a2 = BiCopHfunc2(xl,rep(xl[iq],nq),family=fam2,par=cpar2[1],par2=cpar2[2])
#'     pcint1[iq] = sum(wl*a1)
#'     pcint2[iq] = sum(wl*a2)
#'   }
...
#' @export
#'
rhoS = function(cpar,cop, zero=0, icond=FALSE, tol=0.0001)
{ # Use of pdf has been eliminated for bounded integrand
  if(icond)
  { # cop=pcond (conditional cdf) 
    lb = zero; ub = 1-zero
    spearfn = function(u) 
    { u1 = u[1]; u2 = u[2];
      u1*cop(u2,u1,cpar)
