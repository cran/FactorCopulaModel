# f90 function for input to pdhessminb for 
# combined 1-factor/1-truncated vine  (1f1t)
# BB1 for tree 1, Frank or Gaussian for tree 2
...
#'   models included are BB1 for latent with Frank or Gaussian(bvncop) 
#'    for truncated vine residual dependence
#' 
#' @description
#' negative log-likelihood for 1-factor with tree for residual dependence
#' 
#' @param  param parameter vector (length 2*d+(d-1));
#' BB1 parameters for links of observed variable U_j to latent V, j=1,...,d;    
#' Frank or Gaussian parameters for edge_k=(node_k[1],node_k[2])
#'   observed variables nodek[1],nodek[2] given V, k=1,...,d-1,
#'   where the d-1 edges form a tree.
#'   2*d BB1 parameters (theta_1,delta_1,...,theta_d,delta_d);
#'   d-1 Frank or BVN parameters for residual dependence.
#' @param  dstruct list with nxd data set $data on (0,1)^d ;
#'     $quad is list with quadrature weights and nodes; 
#'     $edg1, $edg2 (d-1)-vectors for node labels of edges 1,...,d-1;
#'       for the tree for residual dependence;
#'     $fam for copula family label, where
#'   $fam=1 for Frank copula for residual dependence conditional on latent;
#'   $fam=2 for bivariate Gaussian/normal copula.
#' @param  iprint indicator for printing of function and gradient
#' 
#' @return nllk, grad, hess (negative log-likelihood, gradient, Hessian at MLE)
#' 
#' @references
...
#' @export
#'
factor1trvine_nllk = function(param,dstruct,iprint=FALSE)
{ udata = dstruct$data
  edg1 = dstruct$edg1
  edg2 = dstruct$edg2
  gl = dstruct$quad
  ...
  wl = gl$weights
  xl = gl$nodes
  fam = dstruct$fam   # 1 for frank, 2 for gaussian/bvncop
  nq = length(xl)
  npar = length(param)
  out = .Fortran("bb1m1f1t", as.integer(fam),
      as.integer(npar), as.double(param), as.integer(d), as.integer(n),
      as.double(udata), as.integer(nq), as.double(wl), as.double(xl), 
