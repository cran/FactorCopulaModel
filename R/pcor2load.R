...
#' Partial correlation representation to loadings for p-factor
#'
#' @param rhomat dxp matrix with correlations for factor 1 in column 1,
#'  and partial correlations with factor k given previous factors in column k
#'
...
#' @export
#'
pcor2load = function(rhomat)
{ if(is.vector(rhomat) | ncol(rhomat)==1) return(rhomat)
  # p>=2
  d = nrow(rhomat)
  p = ncol(rhomat)
  amat = matrix(0,d,p)
  ...
  tem = rep(1,d)
  for(kf in 2:p)
  { temc = sqrt(1-rhomat[,kf-1]^2)
    amat[,kf] = rhomat[,kf]*tem*temc
    tem = tem*temc
  }
  amat
}
